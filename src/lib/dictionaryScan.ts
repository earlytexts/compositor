 ...
 * entry) or unconfirmed (a machine-suggested `?` entry). This module only
 * *locates* those surfaces in the source, so the editor can squiggle each
 * occurrence and offer curation quick-fixes.
 *
 ...
 * sets; because both sides fold identically, matching is exact for ordinary
...
import { accountTokens, type Dictionary, fold } from "@jsr/earlytexts__corpus";
import { documentSourceTokens } from "./hints.ts";

export type UnaccountedStatus = "unaccounted" | "unconfirmed";
 ...
 * unaccounted or unconfirmed, bucketed by status. A surface can only be in one
 * bucket (its entry is present-but-unconfirmed, or absent); a surface accounted
 * at some occurrences and not others (an entry-less word also seen inside
 * markup) lands in `unaccounted` for the occurrences that need it.
 */
export const statusSets = (
  document: MarkitDocument,
  dictionary: Dictionary,
): { unaccounted: Set<string>; unconfirmed: Set<string> } => {
  ...
  for (const token of accountTokens(document, dictionary)) {
    ...
    else if (token.status === "unconfirmed") unconfirmed.add(token.folded);
  }
  return { unaccounted, unconfirmed };
};

/**
 * Every source occurrence of an unaccounted or unconfirmed surface, in reading
 * order. `document` must be the compile of `source`. Returns nothing when the
 * register accounts for everything (the common steady state), so the caller can
 * skip the source walk entirely.
 */
export const scanUnaccounted = (
  ...
  const { unaccounted, unconfirmed } = statusSets(document, dictionary);
  if (unaccounted.size === 0 && unconfirmed.size === 0) return [];
  const out: UnaccountedWord[] = [];
  for (const token of documentSourceTokens(source, document)) {
    const surface = fold(token.display);
    const status: UnaccountedStatus | undefined = unaccounted.has(surface)
      ? "unaccounted"
      : unconfirmed.has(surface)
        ? "unconfirmed"
        : undefined;
